...
Dieser Artikel beschreibt, wie der Logaggregationsdienst Loki mit allen notwendigen Komponenten aufgesetzt und in ein bestehendes System aus Microservices eingebunden werden kann.
...
Promtail ist dafür zuständig, die Logs aus verschiedenen Quellen zu sammeln und an Loki weiterzuleiten.
...
Unter der Portfreigabe sehen wir noch zwei Volumes.
Das erste Volume mounted unsere Konfigurationsdatei in den Container.
Das zweite gibt hier in unserem Beispiel die Systemlogdateien an Promtail weiter.
In der Praxis würden wir hier natürlich die Logdateien unserer Services weitergeben und überwachen.

Hier sehen wir die in den Container gemountete Konfigurationsdatei von Promtail:
```yaml
server:
...
Normalerweise sollte diese Datei auch in einem Docker Volume liegen, aber für unser Testbeispiel liegt die Datei einfach im Container.
Unter `clients` wird die Loki-Instanz konfiguriert, welche Promtail verwenden soll.
...
Eine weitere Alternative, Logs an Loki zu schicken, ist der [loki-logback-appender](https://github.com/loki4j/loki-logback-appender), der in die Logging-Konfiguration der einzelnen Microservices eingebunden wird und dann selbstständig Logs zu Loki schickt.
...
Beispiele für diese Labels sind das Log-Level oder der loggender Service.
Loki kann grundlegend in zwei Modi ausgeführt werden, entweder als einzelner Server oder im Microservice-Modus.
...
Dadurch funktioniert die Namensauflösung und die in Promtail definierten URLs funktionieren.
...
Loki benötigt als Volume einen Ort für die Daten und die Konfigurationsdatei.
...
In unserem Fall verwenden wir das Dateiensystem.
...
Als nächstes konfigurieren wir den `ring`, einen Key-Value-Store, der von Loki intern zur Synchronisation der Instanzen verwendet wird.
...
Grafana benötigt, wie die anderen Dienste auch, ein Volume für die Datenbank und eine Portfreigabe.
...
Also können wir mit folgender docker-compose-Datei Grafana starten:
...
Dazu wählen wir in Grafana unter `Configuration>Data sources>Add Data source` Loki aus.
