...
Wenn man Softwareabhängigkeiten in seinem Projekt aktualisiert, müssen wir trotzdem verantwortungsbewusst sein und prüfen ob die Softwarepakete wie beschrieben funktionieren.
...
Es können viele Jobs in einer Stage existieren, ein Job beschreibt, was zu tun ist, zum Beispiel die Codequalität des Branches überprüfen.
...
Der Job soll ausgeführt werden, wenn ein Commit auf dem Hauptbranch erfolgt.
Das ist nach einem Merge Request der Fall.
...
Wurde ein entsprechendes Versions-Tag gefunden, werden nur alle Commits bis zu diesem Tag untersucht, wurde kein Tag gefunden, werden alle Commits des Projekts untersucht.
...
Der neue Job in der Deploy-Stage soll diese Aufgabe übernehmen:
```yaml
build_docker_image:
    stage: deploy
    variables:
