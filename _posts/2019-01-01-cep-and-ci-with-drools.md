...
Im Anschluss folgen die fachlichen Regeln, die die Ereignisse, im konkreten Fall also die Transaktionen, zeitbasiert untersuchen. Ereignisse müssen über verschiedene „Kanäle“ (sog. Entry-Points) in die BRE eingefügt werden.  Wie im Listing erkennbar ist, wird hier der Entry-Point mit der Bezeichnung `MonitoringStream` verwendet. In der folgenden Zeile wird der zeitliche Zusammenhang beschrieben: gibt es nach dem Eingang der Transaktion, die in der Variable `$tx` gespeichert ist, zwischen 0 und 120 Sekunden (zwei Minuten) ein weiteres Ereignis vom Typ `Transaction` mit gleicher Kartennummer aber unterschiedlichem Ausführungsland, so liegt ein Verdacht auf Kreditkartenbetrug vor und die betroffene Kreditkarte wird unmittelbar gesperrt. 
